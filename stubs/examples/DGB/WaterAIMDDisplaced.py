"""Extracted from DGBTests.test_WaterAIMDDisplaced via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest DGBTests.test_WaterAIMDDisplaced"""
...
class DGBTests(TestCase):
    ...
    def test_WaterAIMDDisplaced(self):
        ...
        steps = 800
        sim = mol.setup_AIMD(pot, initial_energies=[[800 * self.w2h, 0, 1600 * self.w2h]], timestep=25, track_velocities=True)
        sim.propagate(steps)
        ...
        if use_interpolation:
            ...
            interp_data = None
        dgb = DGB.construct(coords, pot if not use_interpolation else interp_data, masses=mol.atomic_masses, transformations='diag', alphas='virial', modes='normal', expansion_degree=2, pairwise_potential_functions={'functions': {(0, 1): self.setupMorseFunction(mol.atomic_masses[0], mol.atomic_masses[1], np.linalg.norm(mol.coords[0] - mol.coords[1]), w=morse_w, wx=morse_wx), (0, 2): self.setupMorseFunction(mol.atomic_masses[0], mol.atomic_masses[2], np.linalg.norm(mol.coords[0] - mol.coords[2]), w=morse_w, wx=morse_wx)}, 'quadrature_degree': 5, 'use_with_interpolation': True}, momenta=momenta, logger=True)
        wfns = self.runDGB(dgb, mol, mode='similarity', plot_spectrum=False, plot_wavefunctions={'cartesians': [0, 1], 'num': 3})
