"""Extracted from DGBTests.test_WaterFromGauss via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest DGBTests.test_WaterFromGauss"""
...
class DGBTests(TestCase):
    ...
    def test_WaterFromGauss(self):
        ...
        if check_anh:
            ...
            VPTRunner.run_simple(TestManager.test_data('h2o_aimd_opt.fchk'), states=2, order=2, degeneracy_specs='auto')
        ...
        if get_initial_velocities:
            ...
            initial_velocities = AIMDSimulator.mode_energies_to_velocities(nms.inverse.T, mol.atomic_masses, [[bend, sym, asym]], inverse=nms.matrix.T)
        ...
        hess = None
        engs = None
        for traj_file in [os.path.expanduser('~/Documents/Postdoc/AIMD-Spec/bomd_h2o_rand.log'), os.path.expanduser('~/Documents/Postdoc/AIMD-Spec/bomd_h2o_rand_5.log'), os.path.expanduser('~/Documents/Postdoc/AIMD-Spec/bomd_h2o_long.log')]:
            with GaussianLogReader(traj_file) as parser:
                traj_data = parser.parse(['AIMDTrajectory'])['AIMDTrajectory']
            c = traj_data.coords
            g = traj_data.vals.gradients
            h = traj_data.vals.hessians
        ...
        coords = nput.vec_tensordot(embedding.coord_data.coords, rots, shared=1, axes=[-1, 2])
        grads = nput.vec_tensordot(grads.reshape(grads.shape[0], -1, 3), embedding.rotations, shared=1, axes=[-1, 1]).reshape(grads.shape)
        hess = nput.vec_tensordot(hess.reshape((hess.shape[0], hess.shape[1], -1, 3)), embedding.rotations, shared=1, axes=[-1, 1]).reshape(hess.shape)
        ...
        if augment_trajectory:
            ...
            nms = mol.normal_modes.modes.basis
            bend, sym, asym = nms.freqs
            sim = mol.setup_AIMD(pot, initial_energies=np.array([[bend, 0, asym], [-bend, 0, asym], [bend, 0, -asym], [-bend, 0, -asym], [bend, sym, 0], [-bend, sym, 0], [bend, sym, 0], [-bend, sym, 0]]) * 0.8, timestep=60)
            sim.propagate(20)
        ...
        dgb = DGB.construct(coords, interp_data, masses=mol.atomic_masses, alphas='auto', transformations='diag', modes='normal', expansion_degree=2, optimize_centers={'method': 'energy-cutoff', 'cutoff': 1000 / UnitsData.hartrees_to_wavenumbers}, logger=True)
        self.runDGB(dgb, mol, similarity_cutoff=0.9, plot_centers=True, plot_spectrum=False, plot_wavefunctions={'cartesians': [0, 1]})
