"""Extracted from DGBTests.test_ModelPotentialAIMD3DHOD via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest DGBTests.test_ModelPotentialAIMD3DHOD"""
...
class DGBTests(TestCase):
    ...
    def test_ModelPotentialAIMD3DHOD(self):
        mol, model = self.buildWaterModel(w2=3869.47 * self.w2h / np.sqrt(2), wx2=84 * self.w2h / np.sqrt(2), atoms=['O', 'H', 'D'], dudr1=1 / 5.5, dudr2=1 / 5.5)
        check_freqs = False
        if check_freqs:
        ...
        if check_anh:
            ...
            VPTRunner.run_simple([mol.atoms, mol.coords], potential_derivatives=model.potential(mol.coords, deriv_order=4)[1:], order=2, states=3, logger=True, degeneracy_specs='auto', calculate_intensities=False, include_coriolis_coupling=True)
            ...
            raise Exception(...)
        sim = model.setup_AIMD(initial_energies=np.array([[5000, 7000, 0], [5000, -7000, 0], [-5000, 7000, 0], [-5000, -7000, 0], [5000, 0, 7000], [5000, 0, -7000], [-5000, 0, 7000], [-5000, 0, -7000]]) * self.w2h * 0.8, timestep=10)
        sim.propagate(3)
        coords = sim.extract_trajectory(flatten=True, embed=mol.coords)
        cartesians = False
        with BlockProfiler(inactive=True):
            dgb = model.setup_DGB(np.round(coords, 8), optimize_centers={'method': 'gram-schmidt', 'norm_cutoff': 1e-14, 'allow_pivoting': True}, modes=None if cartesians else 'normal', cartesians=[0, 1] if cartesians else None, quadrature_degree=3)
            type(self).default_num_plot_wfns = 5
            self.runDGB(dgb, mol, plot_wavefunctions={'cartesians': [0, 1]} if not cartesians else True)
