"""Extracted from DGBTests.test_OCHH via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest DGBTests.test_OCHH"""
...
class DGBTests(TestCase):
    ...
    def test_OCHH(self):
        ...
        if check_anh:
            ...
            VPTRunner.run_simple([mol.atoms, mol.coords], potential_derivatives=[x.reshape((12,) * (i + 1)) for i, x in enumerate(pot_data[1:])], dipole_derivatives=[x.reshape((12,) * i + (3,)) for i, x in enumerate(dip_data)], expansion_order={'dipole': 0}, order=2, states=2, logger=True, degeneracy_specs='auto', calculate_intensities=True, include_coriolis_coupling=True)
            raise Exception(...)
        ...
        if get_anharmonicity:
            ...
            mol.internals = [[0, -1, -1, -1], [1, 0, -1, -1], [2, 1, 0, -1], [3, 1, 0, 2]]
            ...
            for i, j, k in [[0, 1, 0], [1, 2, 1], [1, 3, 3]]:
                ...
                wx = (g / (4 * w)) ** 2 * (internals[3][k, k, k, k] - 5 / 3 * internals[2][k, k, k] ** 2 / internals[1][k, k])
        ...
        run_opts = {'sampling_energy': esamp, 'trajectories': ntraj, 'timestep': ts, 'steps': steps, 'sampled_modes': samp_modes, 'pruning_probabilities': pruning_probabilities, 'momentum_scaling': momentum_scaling, 'use_pairwise': use_pairwise, 'seed': seed}
        ...
        wfn_contours = 10
        plot_centers = False
        plot_styles = {'frame': True, 'ticks': ([-2, -1, 0, 1, 2], [-1, -0.5, 0]), 'axes_labels': ['x (bohr)', 'y (bohr)'], 'padding': ([60, 0], [40, 50]), 'aspect_ratio': 'auto'}
        plot_styles = dict(plot_styles)
        ...
        if plot_dir is not None:
            ...
            plot_dir = os.path.join(plot_dir, '{}_E{}_P{}_nt{}_s{}_ts{}_h{}'.format('interp' if use_interpolation else 'exact', pruning_probabilities[-1][0] if pruning_probabilities is not None else None, esamp, steps, ntraj, ts, param_hash))
        if pruning_probabilities is not None:
            pruning_probabilities = [[e * self.w2h, n] for e, n in pruning_probabilities]
        sim = mol.setup_AIMD(pot, total_energy=esamp * self.w2h, trajectories=ntraj, sampled_modes=samp_modes, timestep=ts, track_velocities=True, seed=seed)
        sim.propagate(steps)
        ...
        if use_interpolation:
            sim_cutoff = 0.8
            sim2 = mol.setup_AIMD(pot, total_energy=esamp * self.w2h, trajectories=5 * ntraj, sampled_modes=samp_modes, timestep=ts, track_velocities=True, seed=seed)
            ...
            pot_spec = pot
        dgb = DGB.construct(coords, pot_spec, masses=mol.atomic_masses, alphas='virial', transformations='diag', optimize_centers=[{'method': 'energy-cutoff', 'probabilities': pruning_probabilities, 'cutoff': None}], modes='normal', expansion_degree=2, dipole_function=dip, kinetic_options=dict(include_diagonal_contribution=True, include_coriolis_coupling=False, include_watson_term=True), pairwise_potential_functions={'functions': {(0, 1): self.setupMorseFunction(mol.atomic_masses[0], mol.atomic_masses[1], np.linalg.norm(mol.coords[0] - mol.coords[1]), w=wCO, wx=wxCO), (1, 2): self.setupMorseFunction(mol.atomic_masses[1], mol.atomic_masses[2], np.linalg.norm(mol.coords[1] - mol.coords[2]), w=wCH, wx=wxCH), (1, 3): self.setupMorseFunction(mol.atomic_masses[1], mol.atomic_masses[3], np.linalg.norm(mol.coords[1] - mol.coords[3]), w=wCH, wx=wxCH)}, 'quadrature_degree': 7, 'use_with_interpolation': True} if use_pairwise else None, momenta=momenta, logger=True)
        ...
        wfns, spec = self.runDGB(dgb, mol, mode='similarity', similarity_cutoff=sim_cutoff, plot_spectrum=True, plot_wavefunctions=plot_wfns, pot_cmap=pot_cmap, pot_points=pot_points, wfn_cmap=wfn_cmap, wfn_points=wnf_points, wfn_contours=wfn_contours, plot_centers=plot_centers, plot_dir=plot_dir, **plot_styles)
        if plot_dir is not None:
