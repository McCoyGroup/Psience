"""Extracted from DGBTests.test_ModelPotentialPhasedAIMD1D via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest DGBTests.test_ModelPotentialPhasedAIMD1D"""
...
class DGBTests(TestCase):
    ...
    def test_ModelPotentialPhasedAIMD1D(self):
        mol, model = self.buildWaterModel(w2=None, wx2=None, ka=None, oh_model=True, dudr1=1 / 5.5, dudr2=None)
        check_freqs = False
        if check_freqs:
        ...
        if check_anh:
            ...
            raise Exception(...)
        sim = model.setup_AIMD(initial_energies=[[-2 * 3701 * self.w2h]], timestep=25, track_velocities=True)
        sim.propagate(15)
        ...
        with BlockProfiler(inactive=True):
            dgb = model.setup_DGB(np.round(coords, 8), optimize_centers=False, modes=None if cartesians else 'normal', cartesians=[0, 1] if cartesians else None, quadrature_degree=25, expansion_degree=2, pairwise_potential_functions={'functions': {(0, 1): self.setupMorseFunction(model, 0, 1)}, 'quadrature_degree': 11}, momenta=1872 / 2 * momenta)
            plot_gaussians = False
            if plot_gaussians:
                self.plot_gaussians(dgb, mol, which=[0, 1, 2, 5], domain_padding=10, scaling=0.2)
                raise Exception(...)
            ...
            self.runDGB(dgb, mol, domain_padding=10, plot_spectrum=False, plot_wavefunctions=True)
