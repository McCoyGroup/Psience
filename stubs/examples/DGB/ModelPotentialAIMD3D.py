"""Extracted from DGBTests.test_ModelPotentialAIMD3D via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest DGBTests.test_ModelPotentialAIMD3D"""
...
class DGBTests(TestCase):
    ...
    def test_ModelPotentialAIMD3D(self):
        mol, model = self.buildWaterModel(dudr1=1 / 5.5, dudr2=1 / 5.5)
        check_freqs = False
        if check_freqs:
        ...
        if check_anh:
            ...
            VPTRunner.run_simple([mol.atoms, mol.coords], potential_derivatives=model.potential(mol.coords, deriv_order=4)[1:], dipole_derivatives=model.dipole(mol.coords, deriv_order=3), order=2, states=3, logger=True, degeneracy_specs='auto', calculate_intensities=True, include_coriolis_coupling=True)
        ...
        if check_dvr:
            ...
            dvr = model.setup_DVR(domain=[[1, 4], [1, 4], [np.deg2rad(60), np.deg2rad(160)]], divs=[800, 800, 800], po_divs=[25, 25, 25], potential_optimize=True, logger=True)
            po_data = dvr.run()
            ...
            raise Exception(po_data.wavefunctions.energies[1] * UnitsData.hartrees_to_wavenumbers, po_data.wavefunctions.frequencies() * UnitsData.hartrees_to_wavenumbers)
        bend, symm, asymm = model.normal_modes()[0]
        init_e = np.array([[-bend, 0, 0], [bend, 0, 0], [0, -symm, 0], [0, 0, asymm]])
        sim = model.setup_AIMD(initial_energies=init_e * 1, timestep=25, track_velocities=True)
        sim.propagate(100)
        ...
        if use_interpolation:
            ...
            potential_data = {'centers': interp_coords, 'values': model.potential(interp_coords, deriv_order=2)}
        else:
            potential_data = None
        use_momenta = True
        use_pairwise = True
        ...
        with BlockProfiler(inactive=True):
            ...
            dgb = model.setup_DGB(coords, potential_function=potential_data, optimize_centers=[{'method': 'energy-cutoff', 'cutoff': pruning_energy} if pruning_energy is not None else None, 1e-14], alphas={'method': 'virial', 'scaling': virial_scaling}, modes=None if cartesians else 'normal', cartesians=[0, 1] if cartesians else None, quadrature_degree=3, expansion_degree=2 if not use_quadrature else None, pairwise_potential_functions={(0, 1): self.setupMorseFunction(model, 0, 1), (0, 2): self.setupMorseFunction(model, 0, 2)} if use_pairwise and (not use_quadrature) else None, transformations='diag', momenta=momenta if use_momenta else None)
            ...
            if plot_gaussians:
                self.plot_gaussians(dgb, mol, cartesians=True, coordinate_sel=[0, 1])
                raise Exception(...)
            ...
            self.runDGB(dgb, mol, similarity_cutoff=0.9, plot_centers=True, plot_spectrum=False, plot_wavefunctions=plot_wfns)
            plt.Graphics().show()
