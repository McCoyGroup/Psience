"""Extracted from DGBTests.test_WaterAIMD via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest DGBTests.test_WaterAIMD"""
...
class DGBTests(TestCase):
    ...
    def test_WaterAIMD(self):
        ...
        if get_anharmonicity:
            ...
            raise Exception(w * UnitsData.hartrees_to_wavenumbers, wx * UnitsData.hartrees_to_wavenumbers)
        check_freqs = False
        if check_freqs:
        ...
        if check_anh:
            ...
            VPTRunner.run_simple([mol.atoms, mol.coords], potential_derivatives=[x.reshape((9,) * (i + 1)) for i, x in enumerate(pot(mol.coords, deriv_order=4)[1:])], order=2, states=3, logger=True, degeneracy_specs='auto', calculate_intensities=False, include_coriolis_coupling=False)
        ...
        if check_dvr:
            ...
            def dvr_pot_2D(r1_r2):
                wtf = mol.get_displaced_coordinates(r1_r2, which=[[1, 0], [2, 0]], internals='convert', shift=False)
                return pot(wtf)[0]
            check_2D = False
            if check_2D:
                from Psience.DVR import DVR
                g = mol.g_matrix
                dvr = DVR(domain=[[-0.8, 4], [-0.8, 4]], divs=[65, 65], potential_function=dvr_pot_2D, mass=[1 / g[0, 0], 1 / g[1, 1]])
                po_data = dvr.run()
            ...
            def dvr_pot(internals):
                return pot(mol.get_displaced_coordinates(internals, which=[[1, 0], [2, 0], [2, 1]], internals='convert', shift=False))[0]
            dvr = model.setup_DVR(domain=[[1, 4], [1, 4], [np.deg2rad(60), np.deg2rad(160)]], divs=[250, 250, 250], po_divs=[10, 10, 10], potential_function=dvr_pot, potential_optimize=True, logger=True)
            po_data = dvr.run()
        ...
        for steps in [500]:
            ...
            svd_min_value = None
            max_overlap_cutoff = 1 - 0.0001
            gs_cutoff = 14
            declustering_radius = None
            sim_cutoff = 0.9
            plot_similarity = False
            pruning_energy = None
            pruning_probabilities = [[1200, 200]]
            ...
            run_opts = {'timestep': timestep, 'trajectories': ntraj, 'pruning_probabilities': pruning_probabilities, 'momentum_scaling': None if not use_momenta else momentum_scaling, 'sqrt_masses': use_sqrt_masses, 'energy_scaling': energy_scaling, 'use_coriolis': use_coriolis, 'use_watson': use_watson, 'ppf_w': None if not use_pairwise_potentials else morse_w, 'ppf_wx': None if not use_pairwise_potentials else morse_wx, 'interpolate_ppf': interpolate_ppf, 'quadrature_degree': quadrature_degree, 'expansion_degree': expansion_degree, 'seed': seed, 'max_overlap_cutoff': max_overlap_cutoff, 'gram_schmidt_cutoff': gs_cutoff, 'similarity_cutoff': sim_cutoff, 'pruning_energy': pruning_energy, 'transformation_method': transformation_method, 'virial_scaling': virial_scaling}
            ...
            if plot_dir is not None:
                ...
                plot_dir = os.path.join(plot_dir, '{}_E{}_s{}_nt{}_ts{}_sim{}_h{}'.format('interp' if use_interpolation else 'exact', pruning_energy if pruning_energy is not None else pruning_probabilities[-1][0] if pruning_probabilities is not None else None, steps, ntraj, timestep, int(sim_cutoff * 100), param_hash))
            if pruning_probabilities is not None:
                pruning_probabilities = [[e / UnitsData.hartrees_to_wavenumbers, p] for e, p in pruning_probabilities]
            ...
            wfn_contours = 10
            plot_centers = False
            plot_styles = {'frame': True, 'ticks': ([-2, -1, 0, 1, 2], [-1, -0.5, 0]), 'axes_labels': ['x (bohr)', 'y (bohr)'], 'padding': ([60, 0], [40, 0]), 'aspect_ratio': 'auto'}
            plot_styles = dict(plot_styles, plot_range=[[-2.5, 2.5], [-1.5, 0.5]], image_size=[500, 500 * 2 / 5])
            np.random.seed(seed)
            dirs = np.random.normal(0, 1, size=(ntraj, 3))
            dirs = dirs / np.linalg.norm(dirs, axis=1)[:, np.newaxis]
            dirs = np.concatenate([[[1, 0, 0], [0, 1, 0], [0, 0, 1]], dirs])
            initial_energies = energy_scaling * dirs * np.array([bend, symm, asym])[np.newaxis, :]
            sim = mol.setup_AIMD(pot, initial_energies=np.array(initial_energies) * energy_scaling, timestep=timestep, track_velocities=True)
            sim.propagate(steps)
            coords, velocities = sim.extract_trajectory(flatten=True, embed=mol.coords)
            momenta = momentum_scaling * velocities * (np.sqrt(mol.atomic_masses[np.newaxis, :, np.newaxis]) if use_sqrt_masses else mol.atomic_masses[np.newaxis, :, np.newaxis])
            ...
            if use_interpolation:
                ...
                if augment:
                    with Checkpointer.from_file(os.path.join(base_dir, interp_traj_file)) as chk:
                        traj = np.concatenate([chk['coords'], coords])
            ...
            with BlockProfiler(inactive=not run_profiler):
                ...
                dgb = DGB.construct(coords, pot if not use_interpolation else interp_data, masses=mol.atomic_masses, alphas={'method': 'virial', 'scaling': virial_scaling}, transformations=transformation_method, optimize_centers=[x for x in [{'method': 'energy-cutoff', 'probabilities': pruning_probabilities, 'cutoff': pruning_energy} if pruning_energy is not None or pruning_probabilities is not None else None, {'method': 'decluster', 'cluster_radius': declustering_radius} if declustering_radius is not None else None, {'method': 'svd', 'contrib_cutoff': svd_contrib_cutoff, 'min_value': svd_min_value} if svd_contrib_cutoff is not None else None, {'method': 'gram-schmidt', 'norm_cutoff': 10 ** (-gs_cutoff), 'max_overlap_cutoff': max_overlap_cutoff} if gs_cutoff is not None else None] if x is not None], modes=None if cartesians else 'normal', cartesians=[0, 1] if cartesians else None, quadrature_degree=quadrature_degree, expansion_degree=expansion_degree, pairwise_potential_functions={'functions': {(0, 1): self.setupMorseFunction(mol.atomic_masses[0], mol.atomic_masses[1], np.linalg.norm(mol.coords[0] - mol.coords[1]), w=morse_w, wx=morse_wx), (0, 2): self.setupMorseFunction(mol.atomic_masses[0], mol.atomic_masses[2], np.linalg.norm(mol.coords[0] - mol.coords[2]), w=morse_w, wx=morse_wx)}, 'quadrature_degree': quadrature_degree, 'use_with_interpolation': interpolate_ppf} if use_pairwise_potentials else None, kinetic_options=dict(include_coriolis_coupling=use_coriolis, include_watson_term=use_watson), momenta=momenta if use_momenta else None, logger=True)
                ...
                wfns = self.runDGB(dgb, mol, mode='similarity', similarity_cutoff=sim_cutoff, plot_similarity=plot_similarity, plot_wavefunctions=plot_wfns, plot_spectrum=False, pot_cmap=pot_cmap, pot_points=pot_points, wfn_cmap=wfn_cmap, wfn_points=wnf_points, wfn_contours=wfn_contours, plot_centers=plot_centers, plot_dir=plot_dir, **plot_styles)
                if plot_dir is not None:
