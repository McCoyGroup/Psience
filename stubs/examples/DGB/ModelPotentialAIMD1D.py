"""Extracted from DGBTests.test_ModelPotentialAIMD1D via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest DGBTests.test_ModelPotentialAIMD1D"""
...
class DGBTests(TestCase):
    ...
    def test_ModelPotentialAIMD1D(self):
        ...
        if check_anh:
            ...
            raise Exception(...)
        sim = model.setup_AIMD(initial_energies=[[3800 * 3 * self.w2h], [-3800 * 3 * self.w2h]], timestep=10)
        sim.propagate(35)
        coords = sim.extract_trajectory(flatten=True, embed=mol.coords)
        cartesians = False
        with BlockProfiler(inactive=True):
            dgb = model.setup_DGB(np.round(coords, 8), optimize_centers=1e-08, alphas={'method': 'virial', 'scaling': 2}, modes=None if cartesians else 'normal', cartesians=[0, 1] if cartesians else None, quadrature_degree=3)
            self.runDGB(dgb, mol, domain_padding=10, plot_spectrum=True, plot_wavefunctions=False)
