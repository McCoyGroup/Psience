"""Extracted from DGBTests.test_NewRunnerOH via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest DGBTests.test_NewRunnerOH"""
...
class DGBTests(TestCase):
    ...
    def test_NewRunnerOH(self):
        ...
        dgb, res = DGBRunner.run_simple(oh, plot_wavefunctions=False, use_interpolation=True, total_energy_scaling=1.5, trajectories=10, timestep=2, propagation_time=35, pairwise_potential_functions={(0, 1): 'auto'})
        return
