"""Extracted from DGBTests.test_ModelPotentialAIMD1DOG via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest DGBTests.test_ModelPotentialAIMD1DOG"""
...
class DGBTests(TestCase):
    ...
    def test_ModelPotentialAIMD1DOG(self):
        mol, model = self.buildWaterModel(oh_model=True, dudr1=1 / 2)
        check_freqs = False
        if check_freqs:
        ...
        cart_dipole_func = lambda coords, deriv_order=None: model.dipole(coords) if deriv_order is None else [np.moveaxis(d, -1, 1) for d in model.dipole(coords, deriv_order=deriv_order)]
        ...
        def r_dipole_func(rs, deriv_order=None):
            rs = rs.reshape(-1, 1)
            if deriv_order is None:
                u = np.moveaxis(np.array([m[0](rs) for m in mu_derivs]), 0, 1)
                return u
            elif deriv_order > 8:
                raise ValueError("only imp'd up to 8th order")
            else:
                u = [np.moveaxis(np.array([m[i](rs) for m in mu_derivs]), 0, 1) for i in range(deriv_order + 1)]
                return u
        coords, sim = self.buildTrajectory(mol, cart_pot_func, 50, timestep=5, initial_energies=[[15000 * self.w2h], [-15000 * self.w2h]])
        ...
        def morse_basic(r, re=re, alpha=a, De=De, deriv_order=None, _morse=PotentialData['MorsePotential']):
            return _morse(r, re=re, alpha=alpha, De=De, deriv_order=deriv_order)
        pairwise_potential_functions = {(0, 1): morse_basic}
        mass_vec = np.array([mol.masses[0] * UnitsData.convert('AtomicMassUnits', 'ElectronMass'), mol.masses[1] * UnitsData.convert('AtomicMassUnits', 'ElectronMass')])
        ...
        self.buildRunDGB(r_crd, r_pot_func, r_dipole_func, alphas='virial', optimize_centers=True, masses=[red_mass], expansion_degree=2, plot_wavefunctions=True, plot_spectrum=False)
        ...
        self.buildRunDGB(coords, cart_pot_func, cart_dipole_func, masses=mass_vec, modes='normal', optimize_centers=True, alphas='virial', expansion_degree=2, pairwise_potential_functions=pairwise_potential_functions, plot_wavefunctions=False, plot_spectrum=False)
        ...
        self.buildRunDGB(np.round(coords, 8), cart_pot_func, cart_dipole_func, masses=mass_vec, cartesians=[0], optimize_centers=True, alphas='masses', expansion_degree=2, pairwise_potential_functions={'functions': pairwise_potential_functions, 'quadrature_degree': 9}, plot_wavefunctions=False)
