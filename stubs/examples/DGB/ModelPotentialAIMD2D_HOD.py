"""Extracted from DGBTests.test_ModelPotentialAIMD2D_HOD via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest DGBTests.test_ModelPotentialAIMD2D_HOD"""
...
class DGBTests(TestCase):
    ...
    def test_ModelPotentialAIMD2D_HOD(self):
        mol, model = self.buildWaterModel(ka=None, atoms=['O', 'H', 'D'], w2=3869.47 * self.w2h / np.sqrt(2), wx2=84 * self.w2h / np.sqrt(2), dudr1=1 / 5.5, dudr2=1 / 5.5)
        check_freqs = False
        if check_freqs:
        ...
        sim = model.setup_AIMD(initial_energies=np.array([[symm, asymm], [-symm, asymm], [-symm, -asymm], [symm, -asymm]]) * 1, timestep=15, track_velocities=True)
        sim.propagate(35)
        ...
        with BlockProfiler(inactive=True):
            dgb = model.setup_DGB(np.round(coords, 8), optimize_centers=1e-14, modes=None if cartesians else 'normal', cartesians=[0, 1] if cartesians else None, quadrature_degree=7, kinetic_options=dict(include_diagonal_contribution=True, include_watson_term=True, include_coriolis_coupling=True), momenta=momenta)
            plot_gaussians = False
            if plot_gaussians:
                self.plot_gaussians(dgb, mol)
                raise Exception(...)
            self.runDGB(dgb, mol, plot_wavefunctions={'cartesians': [0, 1]} if not cartesians else True, plot_spectrum=False)
