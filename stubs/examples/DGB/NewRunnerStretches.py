"""Extracted from DGBTests.test_NewRunnerStretches via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest DGBTests.test_NewRunnerStretches"""
...
class DGBTests(TestCase):
    ...
    def test_NewRunnerStretches(self):
        ...
        dgb, res = DGBRunner.run_simple(water, plot_wavefunctions=3, use_interpolation=True, initial_mode_directions=[[1, 1], [-1, 1], [0, -1], [-1, 0]], timestep=15, propagation_time=25, pairwise_potential_functions={(0, 1): 'auto', (0, 2): 'auto'})
        (wnfs, plots), spec = res
        spec.plot().show()
        return
