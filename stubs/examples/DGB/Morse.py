"""Extracted from DGBTests.test_Morse via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest DGBTests.test_Morse"""
...
class DGBTests(TestCase):
    ...
    def test_Morse(self):
        ...
        domain = [[-0.45, 2]] * d
        ndim = len(domain)
        pts = np.array(np.meshgrid(*(np.linspace(d[0], d[1], n) for d, n in zip(domain, ndivs)))).T.reshape(-1, ndim)
        w = 2.05
        wx = 0.1
        mu = 1
        de = w ** 2 / (4 * wx)
        ...
        wfns = DGB.run(pts, pot, alphas=2, optimize_centers=False, clustering_radius=0.01, min_singular_value=0.0001)
        e = wfns.energies
        ...
        self.assertLess(np.linalg.norm(e[:3] - test_es[:3]), 0.0025, msg='{} != {}'.format(e[:3], test_es[:3]))
