"""Extracted from DGBTests.test_Harmonic via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest DGBTests.test_Harmonic"""
...
class DGBTests(TestCase):
    ...
    def test_Harmonic(self):
        ...
        wfns = DGB.run(pts, pot, alphas=1, clustering_radius=0.05, min_singular_value=0.0001)
        test_es = np.sort(np.sum(list(itertools.product(*[np.arange(8) + 1 / 2] * ndim)), axis=-1))
        self.assertLess(np.linalg.norm(wfns.energies[:8] - test_es[:8]), 0.05)
