"""Extracted from DGBTests.test_ModelPotentialAIMD2DPhased via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest DGBTests.test_ModelPotentialAIMD2DPhased"""
...
class DGBTests(TestCase):
    ...
    def test_ModelPotentialAIMD2DPhased(self):
        ...
        sim = model.setup_AIMD(initial_energies=np.array([[-symm, -asymm / 4], [-symm, asymm / 4], [symm / 1.25, 0], [0, -asymm]]) * 1.5, timestep=35, track_velocities=True)
        sim.propagate(75)
