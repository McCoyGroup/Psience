"""Extracted from DGBTests.test_SampleRotated via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest DGBTests.test_SampleRotated"""
...
class DGBTests(TestCase):
    ...
    def test_SampleRotated(self):
        ndim = d = 2
        reduced_mass = (1 / (16 * UnitsData.convert('AtomicMassUnits', 'ElectronMass')) + 1 / (1.01 * UnitsData.convert('AtomicMassUnits', 'ElectronMass'))) ** (-1)
        ...
        def simple_morse(c, de=de, a=a, deriv_order=None):
            ...
            return res
        test_ham = DGB.construct(np.array([[-1, 1], [1, 0.5]]), simple_morse, alphas=[[1, 0.1], [1, 0.1]], masses=[1], expansion_degree=2, transformations=np.array([np.eye(2), np.array([[1 / np.sqrt(2), -1 / np.sqrt(2)], [1 / np.sqrt(2), 1 / np.sqrt(2)]]).T]))
        self.assertTrue(np.allclose(test_ham.gaussians.coords.centers, np.array([[-1, 1], [1, 0.5]])))
        self.assertTrue(np.allclose(test_ham.gaussians.alphas, np.array([[1, 0.1], [1, 0.1]])))
        self.assertTrue(np.allclose(test_ham.S, [[1, 0.2859716], [0.2859716, 1]]))
        self.assertTrue(np.allclose(test_ham.T, [[0.55, -0.03266702], [-0.03266702, 0.55]]))
        self.assertTrue(np.allclose(test_ham.V, [[2.23291299, 0.66597918], [0.66597918, 0.99361619]]))
