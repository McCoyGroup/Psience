"""Extracted from DGBTests.test_NewRunnerWaterMBPol via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest DGBTests.test_NewRunnerWaterMBPol"""
...
class DGBTests(TestCase):
    ...
    def test_NewRunnerWaterMBPol(self):
        ...
        dgb, res = DGBRunner.run_simple(water, plot_wavefunctions={'cartesians': None, 'num': 5}, use_interpolation=False, initial_mode_directions=[[-1, 0, 0], [0, 1, 0], [0, 0, 1]], timestep=15, propagation_time=25)
        (wnfs, plots), spec = res
        return
