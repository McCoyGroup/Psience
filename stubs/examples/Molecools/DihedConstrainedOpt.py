"""Extracted from MolecoolsTests.test_DihedConstrainedOpt via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_DihedConstrainedOpt"""
...
class MolecoolsTests(TestCase):
    ...
    def test_DihedConstrainedOpt(self):
        ...
        constraints = [(2, 0, 1, 5)]
        meth_opt = methanol.optimize(max_displacement=0.5, max_iterations=10, coordinate_constraints=constraints)
        ...
        print(coordops.zmatrix_unit_convert(coordops.cartesian_to_zmatrix(methanol.coords, methanol_zmatrix).coords, UnitsData.convert('BohrRadius', 'Angstroms'), rad2deg=True))
        print(coordops.zmatrix_unit_convert(coordops.cartesian_to_zmatrix(meth_opt.coords, methanol_zmatrix).coords, UnitsData.convert('BohrRadius', 'Angstroms'), rad2deg=True))
