"""Extracted from MolecoolsTests.test_MolecularSES via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_MolecularSES"""
...
class MolecoolsTests(TestCase):
    ...
    def test_MolecularSES(self):
        base = Molecule.from_string('CO[C@]12C[C@H:1](C=C1)CC2')
        frag = Molecule.from_string('[cH:1]1ccc(F)cc1')
        ...
        new: Molecule = base.attach_functional_group([targ], frag.atoms, frag.coords, frag.bonds, group_site=root)
        surf = new.get_surface(density=2, point_generator='lebedev')
        fig = new.plot(backend='x3d', atom_radius_scaling=0.9)
        mesh = surf.get_triangulation(method='isosurface', probe_radius=2, bbox_scaling=1.5, probe_type='ses', grid_samples=80)
