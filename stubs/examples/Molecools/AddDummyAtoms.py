"""Extracted from MolecoolsTests.test_AddDummyAtoms via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_AddDummyAtoms"""
...
class MolecoolsTests(TestCase):
    ...
    def test_AddDummyAtoms(self):
        ...
        del mol
        self.assertEquals(mol2.atoms, ('H', 'O', 'O', 'N', 'O', 'X'))
        ...
        self.assertEquals(mol2.internal_coordinates[3, 0], 5.0)
        self.assertEquals(mol2.internal_coordinates[3, 1], np.pi / 2)
        self.assertEquals(mol2.internal_coordinates[3, 2], np.pi / 2)
        self.assertEquals(mol2.internal_coordinates[4, 2], np.pi / 2)
        self.assertEquals(mol2.internal_coordinates[5, 2], -np.pi / 2)
