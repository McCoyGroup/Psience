"""Extracted from MolecoolsTests.test_NMFiniteDifference via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_NMFiniteDifference"""
...
class MolecoolsTests(TestCase):
    ...
    def test_NMFiniteDifference(self):
        ...
        def prep_jobs_directory(spec, displacements, jobs):
            ...
        propylbenzene_setup = Molecule.from_file(TestManager.test_data('proplybenz.hess'), energy_evaluator=create_jobs)
        setup = propylbenzene_setup.partial_force_field()

        def parse_jobs():
            ...
        propylbenzene_parse = Molecule.from_file(TestManager.test_data('proplybenz.hess'), energy_evaluator=create_jobs)
        setup = propylbenzene_setup.partial_force_field()
