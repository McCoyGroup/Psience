"""Extracted from MolecoolsTests.test_PysisGSMProfileGenerator via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_PysisGSMProfileGenerator"""
...
class MolecoolsTests(TestCase):
    ...
    def test_PysisGSMProfileGenerator(self):
        ...
        traj: list[Molecule] = Molecule.from_file(TestManager.test_data('traj.xyz'), max_blocks=-1, units='Angstroms')
        init_js = dev.read_json(TestManager.test_data('product.json'))
        new_js = dev.read_json(TestManager.test_data('trajectory.json'))
        traj = [Molecule(init_js['atoms'], c) for c in new_js['final_trajectory']]
        from Psience.Reactions import Reaction
        rxn = Reaction([traj[0]], [traj[-1]])
        prof = rxn.get_profile_generator('ase-neb', energy_evaluator='aimnet2', spring_constant=1, climb=True)
        new_images = prof.generate(base_images=traj)
        traj[0].plot([i.coords for i in new_images]).show()
        new_structs = np.array([i.coords for i in new_images])
        old_structs = np.array(new_js['final_trajectory'])
        f1 = plt.Plot(np.linalg.norm(new_structs[:, 2] - new_structs[:, 3], axis=-1), prof.evaluate_profile_energies(new_images))
        plt.Plot(np.linalg.norm(old_structs[:, 2] - old_structs[:, 3], axis=-1), new_js['final_energies'], figure=f1)
        f1.show()
