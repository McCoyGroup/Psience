"""Extracted from MolecoolsTests.test_HamiltonianExpansions via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_HamiltonianExpansions"""
...
class MolecoolsTests(TestCase):
    ...
    def test_HamiltonianExpansions(self):
        ...
        print()
        tbhp = Molecule.from_file(TestManager.test_data('tbhp_180.fchk'))
        print(mfmt.format_mode_labels(tbhp.get_mode_labels(), tbhp.get_normal_modes().freqs * UnitsData.hartrees_to_wavenumbers))
        return
        print(mfmt.format_symmetric_tensor_elements(tbhp.potential_derivatives[1] * UnitsData.hartrees_to_wavenumbers, cutoff=1000))
        return
        model = TaborCHModel.from_molecule(tbhp, oblique=True)
        ham = tbhp.get_hamiltonian(modes=model.modes)
        print(mfmt.TableFormatter('{:.0f}').format(model.f * UnitsData.hartrees_to_wavenumbers))
