"""Extracted from MolecoolsTests.test_ProjectedLocalModes via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_ProjectedLocalModes"""
    ...
    def test_ProjectedLocalModes(self):
        ...
        def do_the_thing(lms, freqs):
            ...
            for i in range(len(freqs)):
                ...
                disps.append(np.diff(nput.internal_coordinate_tensors(scans, [(0, 1)], order=0)[0][:, 0], axis=0))
