"""Extracted from MolecoolsTests.test_MultiGMatrix via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_MultiGMatrix"""
...
class MolecoolsTests(TestCase):
    ...
    def test_MultiGMatrix(self):
        mol = Molecule.from_file(TestManager.test_data('nh3.fchk'), internals=[[0, -1, -1, -1], [1, 0, -1, -1], [2, 0, 1, -1], [3, 0, 1, 2]])
        other_structs = mol.get_displaced_coordinates(np.random.rand(5, 4, 3) / 5)
