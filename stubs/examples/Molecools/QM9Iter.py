"""Extracted from MolecoolsTests.test_QM9Iter via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_QM9Iter"""
    ...
    def test_QM9Iter(self):
        ...
        for n, data in enumerate(supplier.data_iter(['coords', 'atoms', 'smiles'], start_at=65500)):
            ...
            if n % update_interval == 0:
                import pprint
                pprint.pprint({k: np.average(v).tolist() for k, v in bond_lengths.items()})
