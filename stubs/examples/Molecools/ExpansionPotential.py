"""Extracted from MolecoolsTests.test_ExpansionPotential via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_ExpansionPotential"""
...
class MolecoolsTests(TestCase):
    ...
    def test_ExpansionPotential(self):
        ...
        disps, scan_coords = h2co.get_scan_coordinates([[-0.5, 0.5, 1000]], which=[[2, 2]], return_displacements=True)
        [grad, hess, cubes, quarts] = h2co.potential_derivatives
        vals_2 = h2co.calculate_energy(scan_coords, evaluator=('expansion', {'expansion': [grad, hess]})) * 219475.6
        vals_3 = h2co.calculate_energy(scan_coords, evaluator=('expansion', {'expansion': [grad, hess, cubes]})) * 219475.6
        vals_4 = h2co.calculate_energy(scan_coords, evaluator=('expansion', {'expansion': [grad, hess, cubes, quarts]})) * 219475.6
