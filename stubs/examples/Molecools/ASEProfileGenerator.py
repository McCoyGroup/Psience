"""Extracted from MolecoolsTests.test_ASEProfileGenerator via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_ASEProfileGenerator"""
...
class MolecoolsTests(TestCase):
    ...
    def test_ASEProfileGenerator(self):
        ...
        prof = rxn.get_profile_generator('pys-neb', energy_evaluator='aimnet2')
        new_images = prof.generate(base_images=traj)
        traj[0].plot([i.coords for i in new_images]).show()
        f1 = plt.Plot(prof.evaluate_profile_distances(new_images, normalize=False), prof.evaluate_profile_energies(new_images))
        plt.Plot(new_js['final_rmsds'], new_js['final_energies'], figure=f1)
        f1.show()
