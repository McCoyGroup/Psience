"""Extracted from MolecoolsTests.test_RDKitPlotErrors via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_RDKitPlotErrors"""
...
class MolecoolsTests(TestCase):
    ...
    def test_RDKitPlotErrors(self):
        Molecule.from_string('CO/N=C(\\\\C(O)=N[C@@H]1C(=O)N2C(C(=O)O)=C(C[N+:7]3=[C:6]=[C:5]4CC=C/[C:3]([S:2](=O)(=O)[OH:1])=[C:4]\\\\4C=C3)CS[C@H]12)c1csc(=N)[nH]1').plot(backend='rdkit').show()
