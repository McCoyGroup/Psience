"""Extracted from MolecoolsTests.test_MultiXYZParsing via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_MultiXYZParsing"""
...
class MolecoolsTests(TestCase):
    ...
    def test_MultiXYZParsing(self):
        mol = Molecule.from_file(TestManager.test_data('traj.xyz'), units='Angstroms')
        self.assertEquals(mol.coords.shape, (27, 3))
        traj = Molecule.from_file(TestManager.test_data('traj.xyz'), units='Angstroms', max_blocks=-1)
