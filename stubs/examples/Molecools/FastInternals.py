"""Extracted from MolecoolsTests.test_FastInternals via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_FastInternals"""
...
class MolecoolsTests(TestCase):
    ...
    def test_FastInternals(self):
        sys = 'nh3.fchk'
        mol = Molecule.from_file(TestManager.test_data(sys), internals=[(1, 0), (2, 0), (0, 1, 2), (0, 3), (0, 1, 3), (3, 0, 1, 2)])
        mol2 = Molecule.from_file(TestManager.test_data(sys), internals=[[0, -1, -1, -1], [1, 0, -1, -1], [2, 0, 1, -1], [3, 0, 1, 2]])
        cart = mol.get_cartesians_by_internals(1)[0][0]
        cart3 = mol2.get_cartesians_by_internals(1, strip_embedding=True)[0][0]
        raise Exception([np.round(s1 - s2, 6) for s1, s2 in zip(mol.get_cartesians_by_internals(2), mol2.get_cartesians_by_internals(2, method='classic', strip_embedding=True))])
