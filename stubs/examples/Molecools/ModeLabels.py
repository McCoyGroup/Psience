"""Extracted from MolecoolsTests.test_ModeLabels via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_ModeLabels"""
...
class MolecoolsTests(TestCase):
    ...
    def test_ModeLabels(self):
        ...
        for i, (freq, lab) in enumerate(zip(reversed(modes.freqs), reversed(mode_labs))):
            print('Mode {}: {:.0f} {}'.format(i + 1, freq * UnitsData.hartrees_to_wavenumbers, 'mixed' if lab.type is None else lab.type))
        return
