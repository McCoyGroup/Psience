"""Extracted from MolecoolsTests.test_RenormalizeGaussianModes via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_RenormalizeGaussianModes"""
...
class MolecoolsTests(TestCase):
    ...
    def test_RenormalizeGaussianModes(self):
        with GaussianFChkReader(self.test_HOD) as gr:
            parse = gr.parse(['Coordinates', 'Gradient', 'AtomicMasses', 'ForceConstants', 'ForceDerivatives', 'VibrationalModes', 'VibrationalData'])
