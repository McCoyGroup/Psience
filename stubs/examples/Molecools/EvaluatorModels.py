"""Extracted from MolecoolsTests.test_EvaluatorModels via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_EvaluatorModels"""
...
class MolecoolsTests(TestCase):
    ...
    def test_EvaluatorModels(self):
        ...
        print(dev.str_startswith(None, 'a'), dev.str_startswith(['a'], 'a'), dev.str_startswith('a', 'a'))
        return
        import os
        os.environ['TORCH_COMPILE_DISABLE'] = '1'
        Molecule.from_string('CC[N:1]', add_implicit_hydrogens='full', energy_evaluator='aimnet2:aimnet2nse', spin=1).optimize(max_iterations=100).animate_mode(0).show()
