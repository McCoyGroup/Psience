"""Extracted from MolecoolsTests.test_FragBaseDraw via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_FragBaseDraw"""
...
class MolecoolsTests(TestCase):
    ...
    def test_FragBaseDraw(self):
        bits = Molecule.from_string('FC1CCC(C(=C)C)CC1.C1OCC(C(OC)O)C1C(OC)O', 'smi', confgen_opts=dict(verbose=True, random_seed=12321)).fragments[0].get_embedded_molecule()
        bits.plot(mode='x3d', principle_axes=True, image_size=[500, 500], theme='flat', draw_coords={(0, 1, 2): {'label': '$\\theta$', 'label_style': {'offset_magnitude': 1.1, 'billboard': True, 'color': 'red', 'fontsize': 24}}}, highlight_atoms=[0, 1, 2], view_settings={'view_vector': [0, 0, 1]}).show()
