"""Extracted from MolecoolsTests.test_ProjectedConstrainedModes via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_ProjectedConstrainedModes"""
...
class MolecoolsTests(TestCase):
    ...
    def test_ProjectedConstrainedModes(self):
        methanol = Molecule(['C', 'O', 'H', 'H', 'H', 'H'], [[-0.6988896, 0.00487717, 0.00528378], [1.69694605, -1.08628154, -0.22505594], [-1.27384807, -0.22559494, 1.98568702], [-0.59371792, 2.01534286, -0.59617633], [-2.04665278, -0.99128091, -1.21504054], [2.91616232, 0.28293736, 0.04530201]], energy_evaluator='rdkit').optimize(max_iterations=50)
        fixed_coord = [(2, 0, 1, 5)]
        nms = methanol.get_normal_modes(zero_freq_cutoff=0.0001)
        ...
        def do_the_thing(lms, freqs):
            ...
            for i in range(len(freqs)):
                scans = methanol.get_scan_coordinates([[-75, 75, 3]], which=[i], modes=lms)
                disps.append(np.diff(nput.internal_coordinate_tensors(scans, fixed_coord, order=0)[0][:, 0], axis=0))
