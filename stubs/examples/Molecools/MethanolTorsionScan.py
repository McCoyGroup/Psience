"""Extracted from MolecoolsTests.test_MethanolTorsionScan via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_MethanolTorsionScan"""
...
class MolecoolsTests(TestCase):
    ...
    def test_MethanolTorsionScan(self):
        ...
        methanol = Molecule(['C', 'O', 'H', 'H', 'H', 'H'], [[-0.6988896, 0.00487717, 0.00528378], [1.69694605, -1.08628154, -0.22505594], [-1.27384807, -0.22559494, 1.98568702], [-0.59371792, 2.01534286, -0.59617633], [-2.04665278, -0.99128091, -1.21504054], [2.91616232, 0.28293736, 0.04530201]], energy_evaluator='aimnet2')
        methanol = methanol.optimize()
        me_int = methanol.modify(internals=methanol_zmatrix)
        scan_spec = [-np.deg2rad(180), np.deg2rad(180), 72]
        me_coords = me_int.get_scan_coordinates([scan_spec], which=[11], internals='reembed')
