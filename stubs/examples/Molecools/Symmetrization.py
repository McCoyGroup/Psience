"""Extracted from MolecoolsTests.test_Symmetrization via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_Symmetrization"""
...
class MolecoolsTests(TestCase):
    ...
    def test_Symmetrization(self):
        ...
        new_struct, new_coords, pg = cpmo3m.symmetrize(sel=[0, 1, 2, 3, 4, 10], tol=0.8, return_point_group=True)
        self.assertIsNot(new_struct, None)
        print(nput.distance_matrix(cpmo3m.coords[(0, 1, 2, 3, 4, 10),]) - nput.distance_matrix(new_struct.coords[(0, 1, 2, 3, 4, 10),]))
