"""Extracted from MolecoolsTests.test_SVGBackend via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_SVGBackend"""
...
class MolecoolsTests(TestCase):
    ...
    def test_SVGBackend(self):
        ...
        right_vector, up_vector, view_vector = nput.view_matrix([0, 1, 0], [0, 0, 1], output_order=['x', 'y', 'z']).T
        uuh2.coords = uuh2.coords - uuh2.center_of_mass[np.newaxis]
        fig = uuh2.plot(backend='x3d', image_size=[500, 500], highlight_atoms=[0, 1, 2], draw_coords={(0, 4): {'label': 'r', 'line_color': 'pink', 'label_style': {'color': 'red'}}, (0, 1, 2): {'label': 'O', 'line_color': 'pink', 'label_style': {'color': 'blue'}}}, view_settings={'up_vector': up_vector, 'view_vector': view_vector, 'view_distance': 15})
        fig.show()
        return
