"""Extracted from MolecoolsTests.test_AnimateColors via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_AnimateColors"""
...
class MolecoolsTests(TestCase):
    ...
    def test_AnimateColors(self):
        ...
        water.plot([water.coords, water.coords + 0.5 * UnitsData.convert('Angstroms', 'BohrRadius'), water.coords + 1 * UnitsData.convert('Angstroms', 'BohrRadius')], animation_frame_styles=[{'glow': 'blue'}, {'glow': 'black'}, {'glow': 'red'}], annotation_function=lambda _, i, __: [mplt.Point(surf_points + i * 0.5, vertex_colors=plt.prep_color(palette='coolwarm', blending=np.linspace(0, 1, len(surf_points)) + i * 0.5), point_size=10)]).show()
