"""Extracted from MolecoolsTests.test_Surface2 via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_Surface2"""
...
class MolecoolsTests(TestCase):
    ...
    def test_Surface2(self):
        ...
        fig = mol.plot(backend='x3d', image_size=800, include_save_buttons=True)
        surf = mol.get_surface(density=10, add_intersection_circles=True, extend_intersection_points=False, neighborhood_tolerance={'distance_scaling': 0.5})
        surf.plot().show()
        return
        mol = Molecule.from_string('CCO')
        fig = mol.plot(backend='x3d', image_size=800, include_save_buttons=True)
        surf = mol.get_surface(samples=500)
        tri = surf.get_triangulation(add_intersection_circles=True, extend_intersection_points=True, occlusion_type='complete')
        tri.plot(figure=fig, solid=True, transparency=0.0, line_color=None, color='black', glow='red', invert_mesh=True).show()
        return
