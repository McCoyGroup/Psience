"""Extracted from MolecoolsTests.test_RestrictedZM2 via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_RestrictedZM2"""
...
class MolecoolsTests(TestCase):
    ...
    def test_RestrictedZM2(self):
        ...
        zm = mol.break_bonds([(0, 2), (1, 3)]).get_bond_zmatrix(required_coordinates=[(0, 2), (1, 3), (0, 4, 5, 1)], validate=True)
        _, geoms, _ = mol.modify(internals=zm).relaxed_scan([0, 4, 10], {(0, 2): 1, (1, 3): 1}, max_iterations=0)
        mol.plot(geoms).show()
