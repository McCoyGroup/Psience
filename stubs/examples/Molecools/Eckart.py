"""Extracted from MolecoolsTests.test_Eckart via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_Eckart"""
...
class MolecoolsTests(TestCase):
    ...
    def test_Eckart(self):
        ...
        pax_rot = ref.principle_axis_frame(sel=sel)
        rot_ref = pax_rot.apply(ref)
        self.assertTrue(np.allclose(rot_ref.center_of_mass, [0.0, 0.0, 0.0]))
        transf = scan.eckart_frame(rot_ref, sel=sel)
        tf_test = transf[0].transformation_function
        tf_mat = tf_test.transform
        self.assertTrue(np.allclose(tf_mat @ tf_mat.T - np.eye(3), 0.0))
        ...
        for t, m in zip(transf, scan):
            ...
            self.assertLess(fuckup / len(sel), 0.1, msg='new: {}\nref: {}'.format(new_mol.coords, rot_ref.coords))
        transf = scan.eckart_frame(ref, sel=sel)
        for t, m in zip(transf, scan):
            new_mol = t(m)
            fuckup = np.linalg.norm(new_mol.coords[sel] - ref.coords[sel])
            self.assertLess(fuckup / len(sel), 0.1, msg='new: {}\nref: {}'.format(new_mol.coords, ref.coords))
