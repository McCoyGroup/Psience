"""Extracted from MolecoolsTests.test_ComplexRelaxedScan via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_ComplexRelaxedScan"""
...
class MolecoolsTests(TestCase):
    ...
    def test_ComplexRelaxedScan(self):
        ...
        mol: Molecule = Molecule.from_string('[CH2:1]1[CH2:3]C=C[CH2:4][CH2:2]1', energy_evaluator='rdkit', internals='zmatrix')
        zm = mol.get_bond_zmatrix(required_coordinates=[(0, 2), (1, 3), (2, 4, 5, 3)])
        int_mol = mol.modify(internals=zm)
        _, geoms, _ = int_mol.relaxed_scan([0, 2, 15], {(0, 2): 1, (1, 3): 1}, max_iterations=0, coordinate_constraints=[(2, 4, 5, 3)])
        wtf = int_mol.get_internals(geoms, strip_embedding=True)
        idx = coordops.zmatrix_indices(zm, [(0, 2), (1, 3), (2, 4, 5, 3)], strip_embedding=True)
        print(idx)
        print(wtf[..., idx])
        int_mol.plot(geoms, highlight_atoms=[2, 4, 5, 3], bonds='recompute', include_save_buttons=True).show()
