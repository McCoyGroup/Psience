"""Extracted from MolecoolsTests.test_VisualizeNormalModes via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_VisualizeNormalModes"""
...
class MolecoolsTests(TestCase):
    ...
    def test_VisualizeNormalModes(self):
        ...
        realvibs.visualize(mode='jupyter')
        plot_vibrations = False
        if plot_vibrations:
            nmodes = 1
            mode_start = 0
            g = GraphicsGrid(nrows=2, ncols=nmodes, graphics_class=Graphics3D, plot_range=[[-2, 2], [-2, 2], [-2, 2]], fig_kw=dict(figsize=(17, 5)), tighten=True)
            for i in range(nmodes):
                nms.visualize(step_size=0.1, figure=g[0, i], which=mode_start + i, anim_opts=dict(interval=10))
            for i in range(nmodes):
                realvibs.visualize(step_size=0.1, figure=g[1, i], which=mode_start + i, anim_opts=dict(interval=10))
            g.show()
        self.assertEquals(tuple(np.round(UnitsData.convert('Hartrees', 'Wavenumbers') * nms.modes.freqs, 4)), tuple(np.round(test_freqs, 4)))
