"""Extracted from MolecoolsTests.test_Functionalization via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest MolecoolsTests.test_Functionalization"""
...
class MolecoolsTests(TestCase):
    ...
    def test_Functionalization(self):
        ...
        mol = mol.attach_functional_group([6], fg.atoms, fg.coords, fg.bonds, group_site=3, dihedral=-np.pi / 2)
        mol = mol.attach_functional_group([8], ['F'], [[0, 0, 0]])
        mol = mol.attach_functional_group([11 - 2 + 4], fg2.atoms, fg2.coords, fg2.bonds, group_site=3)
        mol.plot(backend='x3d').show()
