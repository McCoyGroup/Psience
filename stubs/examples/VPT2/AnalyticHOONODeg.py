"""Extracted from VPT2Tests.test_AnalyticHOONODeg via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest VPT2Tests.test_AnalyticHOONODeg"""
...
class VPT2Tests(TestCase):
    ...
    def test_AnalyticHOONODeg(self):
        ...
        runner, _ = VPTRunner.construct(TestManager.test_data(file_name), 1)
        runner.print_Nielsen_frequencies()
        return
        from McUtils.Parallelizers import Parallelizer
        from McUtils.Profilers import Timer, BlockProfiler
        VPTRunner.run_simple(TestManager.test_data(file_name), 2)
        AnalyticVPTRunner.run_simple(TestManager.test_data(file_name), 2)
        return
