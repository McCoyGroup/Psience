...
class ReactionsTests(TestCase):
    ...
    @validationTest
    def test_EmbeddingsIGuess(self):
    ...
    def test_SetupReactants(self):
        ...
        inds = r.reactant_complex.fragment_indices

        p1 = r.reactant_complex.plot(highlight_atoms=inds[0], backend='x3d')
        p2 = r.product_complex.plot(highlight_atoms=inds[0], backend='x3d')
        p1.show()
        p2.show()

    @debugTest
    def test_ReactionZMatrix(self):
        r = Reaction.from_smiles(
            "C=C.C=CC=C>>C1CCC=CC1",
            fragment_expansion_method='centroid',
            min_distance=.4,
        ...
        core_atoms, new_bonds, broken_bonds = r.get_reaction_core()
        backbone = r.product_complex.edge_graph.find_longest_chain()

        print(backbone)
        r.product_complex.plot(highlight_atoms=backbone)

        # coordops.functionalized_zmatrix()

    @validationTest
    def test_NEBZMatrix(self):
        r = Reaction.from_smiles(
            "C=C.C=CC=C>>C1CCC=CC1",
            fragment_expansion_method='centroid',
            min_distance=.4,
    ...
    @validationTest
    def test_VisualizeReactions(self):
    ...
    @inactiveTest
    def test_RunNEB(self):
        ...

    @inactiveTest
    def test_GrowingString(self):
        ...