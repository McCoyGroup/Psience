...
Compute the operator's Taylor-expansion terms in the molecule's normal-mode coordinates, returning the already-canonicalized `self.derivs` directly if they're already mode-basis.
  - `order`: `int | None`
    > the highest derivative order to compute (only relevant when re-expansion through coordinate Jacobians is needed)
  - `logger`: `Logger | None`
    > logger to report progress to; defaults to `self.logger`
  - `:returns`: `list[np.ndarray]`
    > the operator expansion terms
 </div>
</div>
